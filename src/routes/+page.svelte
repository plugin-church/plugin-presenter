<script lang="ts">
	...
	function openMonitor() {
		monitor = window.open(
			...
			'toolbar=no,scrollbars=no,resizable=yes,top=500,left=500,width=800,height=600'
		);
		if (!monitor) return;
		displayLyric(index);
...
<div class="grid grid-cols-2 justify-center w-screen h-screen">
	<div class="flex flex-col items-center justify-center max-w-screen-md gap-2">
		<h1 class="scroll-m-20 text-4xl font-extrabold tracking-tight lg:text-5xl">Plugin Presenter</h1>
		<Textarea
			class="w-max"
			bind:value={content}
			rows={20}
		...
		<div>
			{#if !monitor}
				<Button on:click={openMonitor}>Start</Button>
			{:else}
				<Button class="mr-6" on:click={closeMonitor}>Stop</Button>
				<Button on:click={() => displayLyric(index - 1)} disabled={index == 0}>Previous</Button>
				<Button on:click={() => displayLyric(index + 1)} disabled={parsed.length == index + 1}
					>Next</Button
				>
			{/if}
		</div>
